  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <section className="space-y-4 max-w-4xl">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs sm:text-sm font-semibold tracking-wide">
          <Sparkles className="w-4 h-4" />
          <span>Universal AI Examination Intelligence Engine</span>
        </div>
      ...
      <section className="p-6 sm:p-8 rounded-3xl bg-slate-900/50 border border-slate-800 backdrop-blur-md space-y-8 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
          ...
          <div className="flex items-center space-x-3">
            ...
            <select
              ...
              className="px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-indigo-300 text-sm font-bold focus:outline-none focus:border-indigo-500"
            >
        ...
        <form onSubmit={runExamAnalysis} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* File Upload Zone */}
            <div className="p-6 rounded-2xl bg-slate-800/30 border-2 border-dashed border-slate-700 hover:border-indigo-500/60 transition-colors flex flex-col items-center justify-center text-center space-y-3">
              ...
              <input
                ...
                className="block text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-slate-800 file:text-indigo-300 hover:file:bg-slate-700 cursor-pointer"
              />
              {uploadedFile && (
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full p-4 rounded-2xl bg-slate-800/40 border border-slate-700 text-slate-200 text-sm font-mono focus:outline-none focus:border-indigo-500 leading-relaxed resize-none"
          ...
          <button
            type="submit"
            disabled={analyzing}
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 via-sky-500 to-indigo-500 hover:from-indigo-500 hover:to-sky-400 text-white font-extrabold text-sm sm:text-base tracking-wide shadow-lg shadow-indigo-500/20 flex items-center justify-center space-x-3 transition-all duration-200 disabled:opacity-50"
          >
        ...
        {analysisOutput && (
          <div className="pt-8 border-t border-slate-800 space-y-8 animate-fadeIn">
                ...
                {analysisOutput.classified_questions.map((q: any, idx: number) => (
                  <div key={idx} className="p-5 rounded-2xl bg-slate-800/40 border border-slate-800 space-y-3">
                    <div className="flex justify-between items-center text-xs sm:text-sm">
                      <span className="font-bold text-indigo-400">Question #{idx + 1} ({q.marks} Marks)</span>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-800 text-xs">
                      <span className="px-3 py-1 rounded-lg bg-slate-800 text-slate-300 font-medium">Chapter: {q.classification.chapter}</span>
                      <span className="px-3 py-1 rounded-lg bg-slate-800 text-slate-300 font-medium">Concept: {q.classification.concept}</span>
                      <span className="px-3 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 font-bold border border-indigo-500/20">Bloom: {q.classification.bloom_taxonomy}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {analysisOutput.synthesized_practice_questions?.length > 0 && (
              <div className="p-6 rounded-2xl bg-slate-900 border border-indigo-500/30 space-y-4 shadow-xl">
                <div className="flex items-center space-x-2 text-indigo-400 font-bold text-sm sm:text-base">
                ...
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs sm:text-sm whitespace-pre-line leading-relaxed">
        ...
        {[
          ...
          return (
            <Link
              key={idx}
              href={mod.href}
              className="p-5 rounded-2xl bg-slate-900/50 hover:bg-slate-900/90 border border-slate-800 hover:border-indigo-500/40 transition-all duration-200 space-y-2 block group shadow-md"
            >
